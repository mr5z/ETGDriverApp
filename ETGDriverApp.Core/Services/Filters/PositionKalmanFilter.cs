...
// the local frame.
internal class PositionKalmanFilter
{
    // acceleration uncertainty; how hard the vehicle can plausibly change
    // velocity between updates. Tune against real driving before trusting.
    private const double AccelNoiseMps2 = 1.5;
    ...
    public double? CourseDegrees => SpeedMps < 0.5
        ? null
        ...
        double latitude, double longitude, double accuracyMeters, DateTimeOffset timestamp)
    {
        ...
        var variance = accuracyMeters * accuracyMeters;
    ...
    public double PositionInnovationSigma(double latitude, double longitude, double accuracyMeters)
    {
        ...
        var r = accuracyMeters * accuracyMeters;
    ...
    {
        var (mx, my) = ToLocal(latitude, longitude);
        var r = Math.Max(accuracyMeters * accuracyMeters, 1);
    ...
    public void UpdateVelocity(double speedMps, double courseDegrees, double speedAccuracyMps)
    {
        ...
        var r = Math.Max(speedAccuracyMps * speedAccuracyMps, 0.25);
